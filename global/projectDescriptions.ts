...
export const projectDescriptions: Project[] = [
  ...
  },
  {
    ...
    purpose: `This project is designed to provide my wife with a way to engage her crochet and art community, grow her business and expand her web presence. In the future, she also hopes to offer free and paid-for digital content via this platform (future feature additions).`,
    details: `Relative to itself, this is a frontend-heavy project with plans to incorporate additional backend functionalities as necessary according to the needs of the client (my wife). Next.js was leveraged to create the frontend and backend components which were deployed and hosted via AWS Amplify. Routing was accomplished via Next.js pages and Link components and styling and mobile optimization was completed leveraging Sass modules. The form component is protected via Google ReCAPTCHA v3 and utilizes Nodemailer for sending emails. Given the client required advanced page analytics, Google Tag Manager and Google Analytics were implemented to track events, including certain custom conversion events related to Instagram views and Square store visits. Future plans include additional backend endpoints (leveraging Next.js API endpoints which are written in Node.js), Redux state management, more advanced SEO, and other additions as future needs arise.`,
